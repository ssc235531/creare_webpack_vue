...
config.plugins = [
    ...
    new HtmlWebpackPlugin({
        ...
        // 需要引入entry里面的几个入口，如果entry里面有公共模块，一定要引入
        chunks:['vendors','index'],

        // 要把<script>标签插入到页面的哪个标签里(body|true|head|false)
        inject: 'body',

        // 生成html文件的标题
        title: '',

        // hash如果为ture 将添加hash到所有的包含的脚本和css文件，对于接触cache很有用
        // minify用于压缩html文件，
        // removeComments:true用于移除html中的注释，
        // collapseWhitespace:true用于删除空白符号与换行符
    }),

    // 提取css单文件的名字，路径和生产环境下的不同，要与修改之后的publickPath相结合
    ...
    // 为组件分配ID,通过这个插件webpack可以分析和优先考虑使用最多的模块，并为他分配最小的ID
...
// 为了实现热加载，需要动态想入口配置注入webpack-hot-middleware/client,路径相对于本文件所在的位置

// var devClient = 'webpack-hot-middleware/client';
//为了修改html文件也能实现热加载，需要修改上面的devClient变量，引入同级目录下的dev-client.js
